...
{ENTER(zend_accel_hash_init)
	...
	memset(accel_hash->hash_table, 0, sizeof(zend_accel_hash_entry *)*accel_hash->max_num_entries);
    DEBUG2(HASH,"Initialising hash at %p with %u elements", accel_hash, hash_size);
}
...
{ENTER(zend_accel_hash_update)
	...
	while (entry) {
		if (entry->hash_value == hash_value
            ...
            DEBUG3(HASH, "Hash: key %*s update at %p", key_length, key, entry);
			return entry;
		}
		entry = entry->next;
	}
    ...
    DEBUG4(HASH, "Hash: key %*s %s insert at %p", key_length, key, indirect ? "indirect":"direct", entry);
	return entry;
}
...
{ENTER(zend_accel_hash_find)
	...
	while (entry) {
		if (entry->hash_value == hash_value
			...
			&& !memcmp(entry->key, key, key_length)) {
			if (entry->indirect) {
                DEBUG3(HASH, "Hash: key %*s found, data at %p", key_length, key, ((zend_accel_hash_entry *)entry->data)->data);
				return ((zend_accel_hash_entry *) entry->data)->data;
			} else {
                DEBUG3(HASH, "Hash: key %*s found, data at %p", key_length, key, entry->data);
				return entry->data;
			}
		}
		entry = entry->next;
	}
    DEBUG2(HASH, "Hash: key %*s not found", key_length, key);
	return NULL;
}
	...
	while (entry) {
		if (entry->hash_value == hash_value
			...
			if (entry->indirect) {
                ...
                DEBUG3(HASH, "Hash: key %*s found, data at %p", key_length, key, entry);
				return entry;
			}
		}
		entry = entry->next;
